...
#     sample = draw_circle([300,300],[start+run,100+k],3,4)
...
#     imageio.imwrite(f'experiment/45/{run}_pixels_upper.png', im)
...
#     k = 0
#     for i in range(14):
#         l_im, m_im = draw_line_n_mask((300,300), [start+k,100], np.pi/2, 5, 1.5, 4, generate_dilation_struct(4*aa_scale), aa_scale, contrast_scale=1.0)
#         image = np.maximum(image, l_im)
#         k+=9
#     k=0
#     for i in range(14):
#         # print(start+run+k,100+k)
#         l_im, m_im = draw_line_n_mask((300,300), [start+run+k,100+run], np.pi/2, 5, 1.5, 4, generate_dilation_struct(4*aa_scale), aa_scale, contrast_scale=1.0)
#         image = np.maximum(image, l_im)
#         k+=9
...
#     image = np.maximum(image, sample)
#     # print(start+run+k,100+k)
#     sample = draw_circle([300,300],[start+run+k,100+run],3,4)
#     im = np.maximum(image, sample)
#     # print(np.max(im))

#     imageio.imwrite(f'experiment/vertical/{run}_pixels_upper.png', im)
...
for run in range(0,10,1):
    ...
    k = 0
    for i in range(14):
        l_im, m_im = draw_line_n_mask((300,300), [start+k,200-k], -np.pi/4, 5, 1.5, 4, generate_dilation_struct(4*aa_scale), aa_scale, contrast_scale=1.0)
        image = np.maximum(image, l_im)
        k+=9
    k=0
    for i in range(14):
        # print(start+run+k,100+k)
        l_im, m_im = draw_line_n_mask((300,300), [start+k+run,200-k], -np.pi/4, 5, 1.5, 4, generate_dilation_struct(4*aa_scale), aa_scale, contrast_scale=1.0)
        image = np.maximum(image, l_im)
        k+=9

    sample = draw_circle([300,300],[start-5, 200+5],3,4)
    image = np.maximum(image, sample)
    # print(start+run+k,100+k)
    sample = draw_circle([300,300],[start+k+run-9,200-k+9],3,4)
    im = np.maximum(image, sample)
    # print(np.max(im))
